...
1. **Speak** — each agent formulates a message based on its current beliefs, retrieved memories, and what others said in the previous round.
2. 2. **Observe** — agents listen to one another and store observations in a memory stream.
   3. 3. **Reflect** — after each round, agents generate a high-level reflection and update their beliefs and goals.
      4. 4. **Evolve** — stance histories and belief-drift scores are tracked across every round and trial.
        
         5. After all trials are complete the simulation generates visualizations and an LLM-written consensus summary that synthesizes where the group ended up.
        
         6. The default debate topic is: **"Whether students should use AI tools in education."**
         ...
         9. ```
            ...
            │   ├── convergence_curve.png     # Did the group converge or stay divided over time?
            ...
            This runs **5 trials × 5 rounds** by default and writes output files to the project root.
            ...
            main.py
              ...
              └── run_one_simulation()
                    └── agent.speak()     # Agent formulates a message
                          └── retriever.retrieve()    # Pull relevant memories
                          └── llm.generate_response() # Call OpenAI
                    └── agent.observe()   # Store others' messages in memory
            ...
            **Stance tracking:** Each round an agent classifies its position as `supportive`, `skeptical`, or `balanced`. These labels are chained into a `stance_history` (e.g. `supportive → balanced → skeptical`) and used to measure drift.
